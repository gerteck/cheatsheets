...
\centerline{\includegraphics[width=0.8\linewidth]{basicAuth}}	
\begin{itemize}
...
\centerline{\includegraphics[width=1\linewidth]{PKCKeyExchange}}	
\begin{itemize}
...
\centerline{\includegraphics[width=1\linewidth]{DHKeyExchange}}	
\medskip
\subsubsection{Diffie-Hellman Key-exchange Example}
\centerline{\includegraphics[width=1\linewidth]{DHKeyExchangeExample}}	
...
\end{itemize}
\centerline{\includegraphics[width=0.7\linewidth]{authKeyExchangeMallory}}	
...
\centerline{\includegraphics[width=1\linewidth]{tls2}}
\centerline{\includegraphics[width=1\linewidth]{tls3}}
...
\end{itemize}
\centerline{\includegraphics[width=0.9\linewidth]{addressingSchemes}}
...
\subsubsection{Security Perimeter}
...
\begin{itemize}
\item \textbf{Principle of Least Privilege}: Disallow camera app to access information not needed.
\item \textbf{Compartmentalization}: Host separate services (e.g. fee payment and exam result database or IT and OT) separately, so if one is compromised / fail, does not affect the other.
\item \textbf{Swiss Cheese Model / Defence in Depth}: Multiple layers of protection, such that vulnerabilities in one layer of protection is caught by another layer. 
\item \textbf{Segregation of Duties}: Eliminate single point of failure, assign different components to different persons.
\end{itemize}
\textbf{Example of security perimeter} on Android: No app by default has permission to perform potentially adverse operations. Each app associated with a ``manifest file'' which lists down permissions the app wishes to have. Isolation of apps is a key design consideration, compared to Windows / Mac.

\subsubsection{Terminology}
A principal (or subject) wants to access an object with some operation.
\begin{itemize}
\item \textbf{Principal}: Refers to some (human) user
\item \textbf{Subject}: Entity in system that operates on behalf of the principal (e.g. processes / requests)
\item \textbf{Reference monitor}: Entity that grants or denies access.
\item \textbf{Access}: Access to object classified: Observe (read), Alter (write, delete), Action (execute)
\end{itemize}

\subsubsection{Access Right Policy}
Who decides access rights to an object?
\begin{itemize}
\item \textbf{Discretionary access control}: Owner of object decides rights.
\item \textbf{Mandatory access control}: System-wide policy decides.
\end{itemize}


\subsection{7.2 Access Control Matrix}
We can use a table called the access control matrix to specify the access rights of a 
particular principal to a particular object. This is similar to an adjacency matrix.
\begin{itemize}
\item r: read, w: write, x: execute, s: execute as owner, o: owner
\end{itemize}
Matrix iterates and specifies access rights for all possible pairs of principals and objects.

\subsubsection{Access Control List / Capabilities}
...
\begin{itemize}
\item \textbf{ACL}: Access control list. Stores access rights to particular object as a list, object centered. Easy to know all subjects that can access a specific object, but difficult to find all objects a specific subject can access.
\item \textbf{Capabilities}: Reverses of ACL. Stores list of capabilities for every subject, capabilities as in access rights to an object, defined as an unforgeable token that ives possessor certain rights to an object. Similar but inversed strength / weakness.
\end{itemize}
...
In UNIX file permission, an ACL is used, but unlike the above implementation of the ACL, 
this ACL only specifies the rights for three parties: Owner, Group, World (others). Subjects in the same group have the same access rights.
\begin{itemize}
\item \textbf{Users and Groups}: Groups are ways to classify users. In UNIX, groups can only be created by root. The information on the groups are stored in the file /etc/group. 
\item \textbf{Privileges}: Used to describe access right to execute a process. Considered a form of intermediate control. Aka protection rings. Each ring has a number, a lower rin number has higher privilege. 
\end{itemize}

\subsubsection{Models of Intermediate Control}
...
\begin{itemize}
\item \textbf{Bell-LaPadula Model}: Focuses on confidentiality. No read up (prevent lower level getting higher info). No write down (prevent leakae of info to lower level). ``No `sensitive' info leaking down''.
\item \textbf{Biba Model}: Focus on integrity. No write up (prevent poisonin upper level data). No read down (to prevent subject reading data posioned by lower level subjects). ``No `malicious' info going up.''
\end{itemize}
...
\subsubsection{Terminology}
\begin{itemize}
\item \textbf{Objects}: In UNIX, objects of access control include Files, Directories, Memory Devices, I/O Devices. All these resources treated as files.
\item \textbf{User}: Each user has unique user/login name, UID (numeric user identifier stored in /etc/passwd, and belong to one or more groups.
\item \textbf{Group}: Unique group name, numeric group identifier (GID)
\item \textbf{Principals}: made up of UID and GID.
\item \textbf{Special User}: superuser, UID 0, usually username root.
\item \textbf{Subjects}: Processes, each with own process ID. PID.
\end{itemize}

\subsubsection{Password File Protection: /etc/passwd}
...
\subsubsection{Shadow Password File /etc/shadow}
This is the file that actually contains the hashed passwords. The fields of the entry are:
Login Name, Hashed Password, Date Of Last Password Change, min / max password age, etc. 

\subsubsection{File System Permission}
...
\begin{itemize}
\item \textbf{chmod}: Use chmod command to change file permission bits.
\item In general, the owner of a file and superuser can change permission bits.
\end{itemize}
...
\subsection{Web Security Threat Models}
\begin{itemize}
\item \textbf{Attackers as Another End Systems}: Computers connected to networks referred to as end systems. Attackers can be another end systems, such as malicious web server that lures victim to visit or client who has access to targeted server. 
\item \textbf{Attackers as Man-In-The-Middle}: attacker secretly relays and possibly 
alters the communications between two parties who believe that they are directly 
communicating with each other. (E.g. at IP layer).
\end{itemize}
