...
\item A schedule that is conflict serializable $\implies$ view serializable.
...
\textbf{Transaction Scheduler}: Scheduler receives input actions (read, write commit, abort) and either outputs, postpones (blocks Xact) or rejects (aborts Xact) action. \\
\textbf{Concurrency Control Algorithms}: Includes lock-based concurrency control, timestamp-based concurrency control, multiversion concurrency control, optimistic concurrency control.

\subsection{8.1 Lock-Based Concurrency Control}
Xact requests for appropriate lock on object to access. Shared (S) lock for reading, Exclusive (X) lock for writing.

\subsubsection{2PL: Two Phase Locking}
\begin{itemize}
\item May release locks at any time
\item Once Xact releases a lock, cannot request any more locks.
\item \textbf{Growing/Shrinking phase}: before/after releasing 1st lock.
\item Prevents all anomalies, including phantom read.
\end{itemize}

\subsubsection{Strict 2PL}
\begin{itemize}
\item \textbf{Strict 2PL}: Xact must hold lock until it commits or aborts.
\item 2PL $\implies$ conflict serializable
\item Strict 2PL $\implies$ strict \& conflict serializable.
\end{itemize}

\subsection{8.2 Lock Management}
\textbf{Deadlock Detection}: waits-for graph (WFG)
\begin{itemize}
\item \textbf{Nodes}: active Xacts, \textbf{Edge}: $T_i \rightarrow T_j$, $T_i$ waiting for $T_j$ to release lock.
\item WFG cycle $\implies$ deadlock. Break deadlock by aborting a Xact in cycle. 
\item Alternative to WFG: \textit{timeout mechanism}.
\end{itemize}
\textbf{Deadlock Prevention}: Older $\rightarrow$ Higher priority (smaller timestamp)
\begin{itemize}
\item \textbf{wait-die policy}: lower-priority aborts instead of waiting (less aggressive, younger txns may keep aborting).
\item \textbf{wound-wait policy}: (preemptive) higher priority aborts lower.
\centerline{\includegraphics[width = 0.9\linewidth]{deadlockPrevention}}
\item Restarted xact use original timestamp to avoid starvation.
\end{itemize}

\subsubsection{Lock Conversion}
\begin{itemize}
\item Increases concurrency, only allowed in growing phase.
\item \textbf{Lock upgrade ($UG_i(A)$)}: allowed if no other xact holding shared lock on $A$ and $T_i$ has not yet released any lock. (ensures serializable schedule).
\item \textbf{Lock downgrade ($DG_i(A)$)}: allowed if $T_i$ has not modified $A$ and not released any lock.
\end{itemize}

\subsubsection{Performance of Locking}
\begin{itemize}
\item block/abort mechn. resolve xact conflict but delays, wastes work done. 
\item To increase system throughput: Reducing locking granularity, reduce time lock held, reduce hotspots (freq accessed/modified DB object).
\end{itemize}

\subsection{8.3 ANSI SQL Isolation Levels}
\centerline{\includegraphics[width = 0.95\linewidth]{ANSISQL}}
\begin{itemize}
\item \textbf{short-duration lock}: can be released before commit/abort.
\item \textbf{long-duration lock}: held until xact commits/aborts.
\end{itemize}

\subsection{8.4 Locking Granularity}
\begin{itemize}
\item (coarsest/most granular) databse $>$ relation $>$ page $>$ tuple
\item \textbf{multi-granular} lock: request different granularity.
\item If $T$ holds lock mode $M$ on data granule $D$, then $T$ implicity holds $M$ on data granules finer than $D$.
\end{itemize}
\textbf{$I$-lock (intention)}
\begin{itemize}
\item Before \textbf{acquring any S-/X-lock on $G$}, must acquire $I$-locks on coarser granules in \textit{top-down} manner. $I$-lock sharable with $I$-locks.
\item Limits concurrency: $S$-lock incompatible with $I$-lock.
\end{itemize}
...
\subsection{Transactional Dependencies: ww, wr, rw}
\begin{itemize}
\item \textbf{immediate successor} $\rightarrow$ no $W(x)$ commits betw commits $x_j$, $x_i$.
\item \textbf{dependency serialisation graph, DSG}:
	\begin{itemize}
	\item Schedule of set of committed transactions \{$T_1, ..., T_k$\}
	\item \textbf{nodes}: (committed) transactions, \textbf{edges}: transactional dependencies
	\item e.g. $T_i \dashrightarrow^{wr} T_j$ (dashed line concurrent, solid line non-concurrent)
	\end{itemize}
\item if $S$ is a SI schedule that is not MVSS, then
	\begin{itemize}
	\item There is at least one cycle in DSG(S)
	\item For each cycle in DSG(S), $\exists T_i, T_j , T_k$ such that: \\
•  $T_i \dashrightarrow^{rw} T_j \dashrightarrow^{rw} T_k$ exists \\
•  $T_i$ and $T_k$ may be same txn (eg. write-skew anomaly)
	\end{itemize}
\end{itemize}

%% \centerline{\includegraphics[width = 0.95\linewidth]{SIAnomaly}}


\section{10. Crash Recovery}
\begin{itemize}
\item \textbf{recovery manager} guarantees atomicity and durability: \\
	\hspace{5mm} undo: preserve atomicity (remove effect of aborts) \\
	\hspace{5mm} redo: durability (re-install effects of commits)
\item recovery manager supports 3 ops: commit(T), abort(T), restart.
\item \textbf{failure types}: transaction failures (xact aborts), system crash (violatile mem loss), media failures (non-violatile mem loss)
\item \textbf{steal policy}: can write dirty page to disk before commit
\item \textbf{force policy}: must write all dirty pages to disk at commit.
\end{itemize}
\centerline{\includegraphics[width = 1\linewidth]{stealForce}}

\section{ARIES Recovery Algorithm}
\textbf{Steal; no-force;} assumes \textbf{strict 2PL} for concurrency control.

\subsubsection{Data Structures}
\begin{itemize}
\item \textbf{log file}: sequential file of records in stable storage
\item \textbf{transaction table (TT)}: 1 entry for each active xact \\
 (xact ID, last LSN, C/U status)
\item \textbf{dirty page table (DPT)}: 1 entry per dirty page in buffer pool \\
 (pageID, recLSN) = earliest log record that dirtied page.
\item \textbf{log records}: (type, xact ID, prevLSN, otherinfo) \\
	\hspace{2mm} - \textit{update}: pageID, before-image, after-image\\
	\hspace{2mm} - \textit{compensation} (CLR): pageID, undoNextLSN (ULR's prevLSN), \\
			\hspace{8mm} action to undo (when update described by ULR is undone).\\
	\hspace{2mm} - \textit{commit}: force-write all records $\leq$ r to stable storage \\
			\hspace{8mm} (flush all log records for xact to disk)\\
	\hspace{2mm} - \textit{abort}: create when txn is to be aborted\\
	\hspace{2mm} - \textit{end}: create when all processing for $T$ is completed.\\
	\hspace{2mm} - \textit{checkpoint}: speed up recovery (scan from checkpoint)\\
\end{itemize}

\subsubsection{Implementing abort}
\textbf{Write-ahead logging (WAL)} protocol: do not flush uncommitted update to DB until log record containing its before-image has been flushed to log.
\begin{itemize}
\item each DB page contains \textbf{pageLSN} (LSN of latest update).
\item before flushing db page P, ensure all log records $\leq$ P.pageLSN have flushed to disk
\end{itemize}

\subsubsection{Implementing commit}
\textbf{Force-at-commit} protocol: do not commit xact until the after-images of all its updated records are in stable storage.
\begin{itemize}
\item \textit{commit} LR; xact considered commit if its commit log record has been writtentto stable sotrage.
\end{itemize}

\subsubsection{Implementing restart}
\textbf{1. analysis phase} - TT (active txns) \& DPT (superset of dirty) \\
1.1 initialise TT \& DPT (retrieve ECPLR from BCPLR) \\
1.2 for each $r$ in log file in forward direction/chronological \\
\hspace{10mm} if \textit{end LR}, remove $T$ from TT; continue \\
\hspace{10mm} if \textit{redoable LR} for P and P not in DPT: \\
	\hspace{15 mm} create P's entry in DPT with recLSN=$r$.LSN \\
\hspace{10mm} add or update entry for $T$ in TT: lastLSN = $r$.LSN\\
	\hspace{15mm} if \textit{commit LR}: update status=C
