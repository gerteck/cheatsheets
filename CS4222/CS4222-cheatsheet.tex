...
\begin{itemize}
  ...
  \end{itemize}
  \item \textbf{Link Margin}: Received power minus receiver sensitivity. (Excess above bare min needed)
...
\begin{itemize}
  ...
  \begin{itemize}
    \item \textbf{Effective Isotropic Radiated Power (EIRP)}: total output power including antenna gain: \framebox{\parbox{\dimexpr\linewidth-1\fboxsep-280\fboxrule}{$EIRP = P_{TX}(dBm) + G_{antenna}(dBi)$}} (in direction of antenna main lobe)
  \end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item More efficient (smaller) antenna size, better radio spectrum usage with no overlap, and enables communication among devices sharing the spectrum. (e.g. 2.401 GHz, 2.412 GHz - frequency division multiplexing).
  \end{itemize}
\end{itemize}

\centerline{\includegraphics[width = 1\linewidth]{modulation.jpg}}
\begin{itemize}
  \item \textbf{Modulation Index}: Signifies extent carrier signal modulated w. message signal. Determines strength of modulated signal, impacts bandwidth, power distribution and overall signal quality.
\end{itemize}

\subsection{Amplitude modulation (AM)}
\begin{itemize}
  \item Change amplitude of carrier signal according to the message signal
      \[
...
\begin{itemize}
  ...
  \item Advantages over AM: resilient to interference, spectrum efficiency, remains used for radio broadcast, computer communication.
...
\subsection{Digital equivalent of AM FM and PM modulation}
\begin{itemize}
