...
\centerline{\includegraphics[width=\linewidth]{integralFormulas}}
...
\subsection{Cross (Vector) product}
\begin{center}
    $\vv{a} \times \vv{b} = \abs{\vv{a}} \abs{\vv{b}} \sin \theta \hat{\vv{n}}$
    \\* $\cvv{a_1 \\ a_2 \\ a_3} \times \cvv{b_1 \\ b_2 \\ b_3} 
        = \cvv{a_2b_3 - a_3b_2 \\ -(a_1b_3 - a_3b_1) \\ a_1b_2 - a_2b_1}$
    \newline
    \begin{multicols}{2}
...
\textbf{Sphere}: Normal Vector: Passes through centre of sphere
...
For $f(x, y)$, (some function of 2 variables)
\begin{center}
...
\subsubsection{Type I: Integrate against complicated y first}
\begin{center}    
    \begin{multicols}{2}
...
\subsubsection{Type II: Integrate against complicated x first}
\begin{center}    
    \begin{multicols}{2}
...
\end{center}
Where, $D = \{ (r, \theta)$ $|$ $0 \leq \theta \leq \_\_ , 0 \leq r \leq \_\_ \} $
...
\subsection{Separable Equations (dx/dy separable)}
\begin{center}
...
\begin{center}
    ...
    \begin{tabular}{|>{\color{black}}c | >{\color{black}}c|}
        ...
        \\~\\$y' + P(x)y = Q(x)y^n$ } 
        	& \makecell{
	            set $z = y^{1-n}$
	            \\ sub $z$ and $z'$ obtain eqn
	            \\ $\Then y' = \frac{y^n}{1-n} z'$
	            \\ $R = e^{\int P dx}$ 
	            \\ $\Then z = \frac{1}{R} \int RQdx$ 
	        }
        \\[4.5ex] \hline
