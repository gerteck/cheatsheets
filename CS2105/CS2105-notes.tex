...
\centerline{\includegraphics[width=0.4\linewidth]{internetstructure}}
\end{itemize}
...
Build reliable transport layer protocol on top of unreliable communication.
\begin{itemize}
\item Factors: \textbf{Packet corruption, loss, reordering, Packet (Long) Delay.}
...
\begin{itemize}
\item Assume: Underlying channel may flip bits. Receiver uses \textbf{stop and wait} protocol. 
\item Receiver uses checksum to detect bit errors, sends NAK if corrupted. Sender resends if NAK received.
\item \textbf{Problem: If ACK or NACK corrupted}, no guaranteed way to recover. If packet re-sent, the receiver cannot identify duplicate.
\end{itemize}
...
\item Receiver includes seq. no, duplicate ACKs results in retransmit current pkt.
\end{itemize}
\smallskip
...
\begin{itemize}
\item Internet: \textbf{“network-of-networks”}, hierarchy of \textbf{Autonomous Systems} (AS), e.g., ISPs, each owns routers and links. Decentralized administration, so routing done hierarchically.
...
\begin{itemize}
\medskip
\centerline{\includegraphics[width=0.6\linewidth]{ipv4datagram}}
...
\begin{itemize}
\item Different links, different MTU (Max Transfer Unit, max amt of data link-level frame can carry). “Too large” IP datagrams may be fragmented by routers.
\centerline{\includegraphics[width=0.9\linewidth]{ipfragmentation}}
\medskip
\item \textbf{Flag}(frag flag) is set to 1 if next fragment from same segment, 0 if this is the last fragment. \textbf{Offset} is in expressed in unit of 8-bytes.
\end{itemize}
...
\begin{itemize}
\item  \textbf{Link layer} sends datagram between adjacent nodes (hosts or routers) over a single link. Responsible for transfer of datagram from node to \underline{physically} \underline{adjacent} node over link. 
\item \textbf{Frames (layer 2 packet)}: IP datagrams encapsulated in link-layer \textbf{frames} for transmission.
...
\item \textbf{Checksum Recap}: treat segment contents as 16 bit int sequences, get 1s complement of sum of segment contents.
\end{itemize}
...
\end{itemize}
\smallskip
\centerline{\includegraphics[width=0.8\linewidth]{interconnectNodes}}
...
\begin{itemize}
\item For CSMA and CSMA/CD above, need minimum frame size so that collisions can always be detected. Ethernet has min size of 64 bytes.
\end{itemize}

\subsubsection{CSMA / Collision Avoidance (CSMA/CA)}
\begin{itemize}
\item Collision detection hard for wireless LANs, as energy levels drop too quickly.
...
\item Used to send, receive link layer frames, when adapter receives frame, check if destination MAC address of frame matches own MAC address.
\item If yes, adapter extracts enclosed datagram and passes it to the protocol stack. Else if no, adapter discards frame w/o interrupting host.
	\begin{itemize}
...
\centerline{\includegraphics[width=0.7\linewidth]{Lan}}
\subsubsection{Ethernet}
\centerline{\includegraphics[width=0.8\linewidth]{ethernet2}}
...
\centerline{\includegraphics[width=0.8\linewidth]{ethernetframe}}
\begin{itemize}
...
\begin{itemize}
\item \textbf{Bus Topology}: (broadcast LAN) All nodes connected, can collide with each other.
\item \textbf{Star Topology}: Switch / Hub in centre, nodes connected to switch. Do not collide with each other.
\end{itemize}

\smallskip
\centerline{\includegraphics[width=0.7\linewidth]{topo}}
...
\medskip
\centerline{\includegraphics[width=0.7\linewidth]{ethernetCSMACD}}
...
\end{itemize}
\medskip
\centerline{\includegraphics[width=0.5\linewidth]{ethernethubswitch}}
...
\end{itemize}
\medskip
\centerline{\includegraphics[width=0.9\linewidth]{switchforwardingtable}}
...
\begin{itemize}
\item \textbf{Confidentiality}: Only sender \& receiver should understand message contents.
\item \textbf{Authentication}: Ability of sender, receiver to confirm identity of each other.
...
\begin{itemize}
\item Alice, Bob and Trudy (Intruder). Eavesdrop, Insert or delete messages.
\item Impersonation or spoofing (fake) source address
\item Hijacking connection, Denial of service.
\end{itemize}

\subsection{Principles of Cryptography}
\subsubsection{Terminology}
\begin{itemize}
\item \textbf{Plaintext}: The original message, \textbf{Ciphertext}: The encrypted message
...
\item \textbf{Known Plaintext Attack}: When have ciphertext and corresponding plaintext.
\item \textbf{Chosen Plaintext Attack}: Presumes attacker can obtain ciphertext for arbitrary plain text
\end{itemize}
...
\end{itemize}
\centerline{\includegraphics[width=0.7\linewidth]{asymmetric}}
...
\item Choose $d$ such that $ed -1$ is exactly divisible by $z$. ($ed$ mod $z = 1$).
...
\item \textbf{Only authorized access inside network:} authenticated users/hosts.
\item UDP: generally firewall either filters all or no UDP.
\item \textbf{Tradeoff}: between security, degree of communcation with outside Internet.
\item \textbf{Limitation}: IP spoofing, router cant verify data if comes from claimed source.
...
\item source IP addr, destination IP addr, TCP/UDP source, destination port numbers
\item ICMP message type, TCP SYN and ACK bits
\end{itemize}
\smallskip
\textbf{Examples:}
\begin{itemize}
...
Stateful firewalls filter packets based on full context of connection, track status of every TCP connection, e.g. connection setup, teardown, etc. Use concept of state table that stores state of legitimate connections. If inactive connections timeout at firewall, incoming packets rejected. 
\\ \textbf{Access Control List (ACL)}:
\centerline{\includegraphics[width=0.7\linewidth]{ACL}}

\subsubsection{Application Gateways}
\begin{itemize}
\item \textbf{Application gateways}: more intelligent firewall systems, know details of apps that generate packets. Also work as proxy servers between client computer and real server. (Different gateway needed for each application.)
...
\centerline{\includegraphics[width=0.9\linewidth]{secureemail1}}
\smallskip
\centerline{\includegraphics[width=0.7\linewidth]{secureemail2}}

\section{7. Multimedia Networking}
\textbf{Multimedia network application}: any network app that employs \textbf{audio or video}. 
\begin{itemize}
...
\item \textbf{Streaming} stored audio, video: can begin playout before downloading entire file.
...
\end{itemize}
\centerline{\includegraphics[width=0.75\linewidth]{vidcompress}}
...
\item \textbf{Concatenation \& Decoding}: All values represented as bits, samples are concatenated. To decode, convert back to analog signal, which is approximate of original (quality reduction), as certain sounds lost in sample and encode.
\end{itemize}
...
\item \textbf{Initial playout delay tradeoff} btw likelihood of buffer starvation \& initial delay.
\end{itemize}
\centerline{\includegraphics[width=0.75\linewidth]{clientbuffer}}

\subsection{UDP Streaming}
\begin{itemize}
\item \textbf	{Push-based Streaming}: Server transmit client’s consumption rate. UDP has no congestion control mechanism, transmit w/o rate-control restrictions of TCP.
\item E.g. video consumption rate $=$ 2Mbps, each UDP packet carry 8,000 video bits. Server must transmit one UDP packet into socket every $\frac{(8000 bits)}{(2 Mbps)} = 4$ msec.
...
\begin{itemize}
\item If application buffer full, send rate reduced to consumption rate. If video paused, sending, buffering continues till buffer full till video resumed.
...
\item \textbf{Drawback}: Fill rate fluctuates due to TCP congestion control \& retransmissions (in-order delivery). Larger playout delay than push-based UDP.
\end{itemize}
\medskip
\centerline{\includegraphics[width=0.85\linewidth]{TCPcongestion}}
...
\end{itemize}
\centerline{\includegraphics[width=0.85\linewidth]{DASH}}
...
\item \textbf{Streamlets or Byte Range}: Client either GET request for specific streamlet, (if server split video into streamlets during pre-processing), or use GET requests with specified byte range in header to get right chunk.
...
\end{itemize}
\centerline{\includegraphics[width=0.8\linewidth]{CDN}}
...
\centerline{\includegraphics[width=0.85\linewidth]{command3}}
...
\subsubsection{Each network interface card has an MAC address. Why not simply use this MAC address for routing of packets on the internet?}
\begin{itemize}
\item An IP address logically comprises two parts: network prefix and host ID. This is 
designed to facilitate routing: routers check prefix and deliver a packet to an 
aggregated destination network. If MAC address is used instead, hierarchical 
routing cannot be achieved. For example, MAC address is burnt in ROM and usually 
cannot be changed. When people carry their laptops around the world, devices in a 
subnet won’t have common prefix in MAC addresses. This makes routing difficult 
as routers have to remember routing for every single MAC address
\end{itemize}

\subsubsection{Local Broadcast Address}
\begin{itemize}
\item IP address 255.255.255.255 is known as local broadcast address. Local broadcast address is used to communicate with all the hosts of the same subnet. Technically, the broadcast should be sent to all the IP addresses that exist. However, it actually serves as an address for the broadcast within the local network as a router does not forward such a packet. Local Broadcast IP address is also known as Limited Broadcast IP address.
\item When the destination IP address is 255.255.255.255 then the destination MAC address is ff.ff.ff.ff.ff.ff which makes this packet both a Layer 2 and a Layer 3 broadcast.
\end{itemize}

\subsubsection{Switches}
\begin{itemize}
\item Switches will forward broadcast traffic to all interfaces, except the one where it originated from. 
\end{itemize}


\subsubsection{Briefly explain why the internet is designed as packet-switching network instead of circuit switching network?}
\begin{itemize}
\item Resource Sharing: In a packet-switching network, multiple communication sessions can share the same network infrastructure concurrently. This contrasts with circuit-switching, where a dedicated circuit is reserved for the entire duration of a communication session, leading to potential underutilization of resources.
\end{itemize}

\subsubsection{Give a reason a stop and wait protocol would have low throughput over a satellite communication link}
\begin{itemize}
\item Experience low throughput over a satellite communication link due to inherent characteristics of satellite communication. (high propagation delay).
\item Signals must travel vast distances. The propagation delay cause the stop-and-wait protocol to be idling, waiting for (ACK) before sending the next packet, making it inefficient as waiting time can be substantial, leading to underutilization of the available bandwidth and low throughput.
\end{itemize}

\subsubsection{Why are internet checksums used in transport layer and network layer while CRCs are used in the link layer?}
\begin{itemize}
\item  Internet checksums, are simple error-checking codes provide a lightweight mechanism for error detection without introducing significant computational overhead. Focus is on detecting errors in the data as it travels from the source to the destination.
\item  The link layer is closer to the physical transmission medium, where errors, including bit errors, are more likely to occur due to noise and interference. CRCs commonly used at the link layer. More robust than simple checksums and can detect wider range of errors. Better suited for detecting errors in the binary stream of data at the link layer.
\end{itemize}

\subsubsection{Why CSMA has better performance than pure aloha, similarly, why CSMA-CD outperforms CSMA}
\begin{itemize}
\item \textbf{CSMA (Carrier Sense Multiple Access) vs. Pure ALOHA:} In CSMA, station listens to channel before transmitting, avoiding collisions with ongoing transmissions, reducing the chances of collisions, making more efficient use of available bandwidth. In Pure ALOHA, stations transmit w/o checking for ongoing transmissions, leading to a higher collision probability and lower overall efficiency.
\item \textbf{CSMA-CD (Carrier Sense Multiple Access with Collision Detection) vs. CSMA}: CSMA-CD Advantage: CSMA-CD includes collision detection, continues to listen to the channel while transmitting, if collision detected, stops transmission immediately, reducing the time wasted on unsuccessful transmissions. 
\end{itemize}


\centerline{\includegraphics[width=0.9\linewidth]{q4}}
\begin{itemize}
\item Encrypted hash of the message: digital signature of Alice used to prove her identity to 
Bob. Alice’s private key is used.
\item Encrypted message: message encrypted with the session key to ensure confidentiality 
of the message. Session key is a symmetric key.
\item Encrypted session key: session key encrypted with Bob’s public key. The purpose is to 
share the session key with Bob
\end{itemize}


\subsubsection{2-D Parity Bits}
\begin{itemize}
\item 2-D parity bits can detect and correct any one-bit (or odd) error. And can detect (but not correct) any two-bit error.
\end{itemize}

\subsubsection{VoIP application:}
\begin{itemize}
\item Receiver side two types of data losses may occur: delay loss and network loss.
\end{itemize}

\subsubsection{ Public key cryptography uses both public and private keys. Let Alice’s public 
key and private key, and let Bob’s public and private key be. Alice sends  message m to Bob. Describe how to ensure message confidentiality and integrity using only these 4 keys}
\centerline{\includegraphics[width=0.85\linewidth]{q4b}}


\subsubsection{Port Numbers}
\begin{itemize}
\item Port numbers range from 0 to 65535. ($2^{16}$)
\item For a NAT router with a single IP address, if each row of NAT translation table is 8B (two IP addresses), then take 524,288 bytes to maximise.
\end{itemize}
