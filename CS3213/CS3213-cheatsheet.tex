...
\begin{itemize}
\item \textbf{Soundness}: A sound type system never accepts a program that can `go wrong.' No false negatives: The language is type-safe. Ensures execution never reaches a "meaningless" state.
\item \textbf{Completeness}: A complete type system never rejects a program that cannot go wrong. No false positives.
