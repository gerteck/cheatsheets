...
\item \textbf{Tarantula Algorithm}: Assigns suspiciousness scores to program elements based on involvement in failing and passing test cases.
	\begin{itemize}[itemsep=-1pt]
	\item Formula:	[\text{Suspiciousness}(line) = $\frac{\%failed(line)}{\%failed(line) + \%passed(line)}$	]
