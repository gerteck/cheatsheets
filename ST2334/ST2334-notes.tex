...
\textbf{Random variable}: A function X, which assigns a real number to every s $\in$ S. \\
...
\subsubsection{CDF Discrete RV: Step Function $F(x)$}
	...
	\centerline{\includegraphics[width=0.6\linewidth]{cdfstep}}
    \begin{itemize}
...
\subsubsection{CDF Continuous RV: $F(x)$}
    \centerline{\includegraphics[width=0.90\linewidth]{cdfcontinuous}}
    \begin{itemize}
...
\begin{itemize}
    \item Consider more than 1 RV simultaneously. Given sample space $S$. Let $X$ and $Y$ be functions mapping $s \in S \to \mathbb{R}$: $(X, Y)$ is 2D random vector.
    \[\text{\textbf{Range:}} R_{X, Y} = \{(x,y) | x = X(s), y=Y(s), s \in S\}\]
...
\textbf{Discrete Distributions}: Whole classes of discrete RVs that arise frequently.
...
\begin{itemize}
    \item \textbf{Bernoulli Trial}: Random experiment w 2 possible outcomes (success or fail).
    ...
    \item \textbf{Bernoulli Process}: Sequence of repeat \underline{independent, identical} B. trials.
...
\centerline{\includegraphics[width=0.9\linewidth]{poissonex}}
...
\medskip
\centerline{\includegraphics[width=0.95\linewidth]{s2}}
...
\centerline{\includegraphics[width=1\linewidth]{tdist2}}
\vfill \null
\columnbreak
...
\end{itemize}
\centerline{\includegraphics[width=0.6\linewidth]{pointintervalestimate}}
...
\end{itemize}
\smallskip
\centerline{\includegraphics[width=0.6\linewidth]{unbiasedestimator}}
\textbf{Example}
\begin{itemize}
...
\begin{itemize}
\item Recall if population normal or sufficiently large, $\frac{\bar{X} - \mu}{\sigma / \sqrt{n}}$ follows (approx) standard normal distribution. 
\end{itemize}
...
\end{itemize}  
\smallskip
\centerline{\includegraphics[width=0.8\linewidth]{Za}}
\textbf{Then, we have}
...
\centerline{$= 1 - \alpha$}
\smallskip
\centerline{\includegraphics[width=0.8\linewidth]{Za2}}
\textbf{Hence}: \\
...
\begin{enumerate}
    ...
    \centerline{\includegraphics[scale=0.4]{rejection-region}}
    \begin{itemize}
    ...
    \end{itemize}
	\medskip
    \centerline{\includegraphics[scale=0.38]{p-value}}
    \item 
    \begin{itemize}
...
\textbf{Two-sided hypothesis test} procedure equivalent to finding \textbf{$100(1 - \alpha) \%$ confidence interval} for $\mu$. 
\begin{itemize}
\item When confidence interval contains $\mu_0$, $H_0$ will not be rejected at level $\alpha$.
\item Similarly, when confidence interval does not contain $\mu$, then $t$ falls within rejection region and so $H_0$ will be rejected.
\end{itemize}
...
\subsubsection{Rejection Regions and p-Values}
    \centerline{\includegraphics[scale=0.25]{comparing-means}}
...
\centerline{\includegraphics[width=0.75\linewidth]{geometricseries}}
...
\centerline{\includegraphics[width=0.9\linewidth]{criticalvalues2}}
\centerline{\includegraphics[width=0.9\linewidth]{criticals2}}

\columnbreak

\centerline{\includegraphics[width=0.8\linewidth]{chitable}}

\subsection{Properties of P.d.f. \& C.d.f}
\smallskip
\centerline{\includegraphics[width=1\linewidth]{pdfcdf1}}
\smallskip
\centerline{\includegraphics[width=1\linewidth]{pdfcdf2}}
\smallskip
\centerline{\includegraphics[width=1\linewidth]{tDistTable}}
