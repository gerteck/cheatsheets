...
\begin{itemize}
	...
	\item \code{<? super Integer>}  $\implies$ inferred as \code{Object} (supertype of Integer)
	\item \code{<? extends Integer>} $\implies$ inferred as \code{Integer}
...
An immutable class $\rightarrow$ an instance cannot have any visible changes outside its abstraction barrier.

\subsubsection{Making Immutable Classes}
\begin{itemize}
\item \code{private final}: We make the class itself \code{final} to disallow inheritance and overriding. Note the the \code{final} keyword prevents assigning new value to the field, but does not prevent the field from being mutated. \textbf{final not sufficient for immutabilitiy.}
\item We return a new instance every time to prevent mutating the current instance. (Copy on write semantic allows us to avoid aliasing bugs without creating excessive copies of objects.)
\end{itemize}
\textbf{Advantages of Immutability:}
\begin{itemize}
\item \textbf{Ease of Understanding \& Safe Sharing:} We are sure that an object’s properties are unchanged unless explicitly re-assigned. Allows us to safely share instances of the class and reduces need to create multiple copies of the object.
\item \textbf{Saves Space}: Share references until instances need to be modified (which creates a new copy)
\item \textbf{Safe Concurrent Execution:} Allows multiple threads of code to run in interleaved fashion, without objects being changed.
\end{itemize}


\subsubsection{Varargs (variable arguments)}
\begin{itemize}
\item Java 5 Introduced the concept of varargs, \textbf{a method parameter of variable length}.
\item will be passed to the method as an array of items.
\item \code{public void of(T... items)\{\}} $\rightarrow$ items will be \code{T[]}
\item \code{@SafeVarargs} annotation if \code{T} is a generic type.
\end{itemize}

\section{Nested Classes: Static Nested \& Inner Classes}
\textbf{Static nested, inner class}: Java allows us to define a class within another class/method. \textbf{Nested Classes} are used to group logically relevant classes together. Nested class is a field of the containing class, and and can access fields and methods of the container class.
\begin{itemize}
\item \code{static nested class} $\rightarrow$ associated with the containg class. Can only access \textbf{static} field / methods of containing class.
\item \code{inner class (non static)} $\rightarrow$ associated with an instance. Can access all fields/methods of containing class.
\end{itemize}

\subsubsection{qualified \code{this}}
e.g. \code{A.this.x} : Used to reference container classes' variables. Otherwise we can't.
\begin{lstlisting}
class A {
 private int x;
...
\section{Local Class (Class in Methods)}
\begin{itemize}
\item class defined within a method, like a local variable.
\item can access class and instance variables from the enclosing class (use qualified \code{this}) + local variables of enclosing method.
\item \code{effectively final} $\rightarrow$ variable does not change after initialisation.
\item \textbf{Will not compile} if variables accessed are NOT effectively final.
 
\subsubsection{Variable Capture}
\item When method returns, the local variables are removed from stack. But an instance of that local class may still exist.
\item The local class, as it can access the local variables in the enclosing method, makes a copy of the local variables inside itself. We say that the local class captures the local variables.
\item In order to prevent bugs, Java only allows a local class to access variables that are explicitly declared final or implicitly final . (effectively final).
\end{itemize}
...
\begin{itemize}
\item \textbf{Format:} \code{new Constructor(arguments) \{ body \} }, or \code{new ( className implements interface) (arguments) \{ body \} }
\item Cannot implement more than one interface, Cannot extend a class and implement an interface at same time.
\item Same rule as local class for variable access.
\end{itemize}


\subsubsection{Stack \& Heap:}
\begin{itemize}
\item \textbf{Static Class:} Java RT: Static fields in memory metaspace. 
\centerline{\includegraphics[width = \linewidth]{staticClassStackHeap}}

\item \textbf{Local Class:}
Local Class: Variables in method will be captured.
\centerline{\includegraphics[width = \linewidth]{localClassStackHeap}}
\centerline{\includegraphics[width = \linewidth]{local2ClassStackHeap}}

\item \textbf{Anonymous Class}
\centerline{\includegraphics[width = \linewidth]{anonClassStackHeap}}


\end{itemize}

\section{8. Functions (F. Programming)}
\subsubsection{Functions}
\begin{itemize}
\item Idea of Function: Some mapping from domain to codomain. It has to be deterministic, immutable, so that we can pass these methods around as objects.
\item \textbf{Cross state barrier:} with functions, you can now pass in functions into a class to use instead of making a new class method for every time of data manipulation.
\item \textbf{Functions as first class citizens:} functions are treated like any other variables, passed around.
\item \textbf{Referential Transparency:} if $f(x) = y$, then any $y$ can be substituted with $f(x)$.
\item \textbf{Pure Function}: No side effects (e.g. printing, changing value, throwing exceptions), every input mapped to an output (null is not within codomain), deterministic, and must return a value.
\end{itemize}

\subsubsection{Functional Interface:} 
Conceptually, a functional interface has exactly one abstract method. We annotate function interfaces with \code{@FunctionalInterface} annotation.
\begin{lstlisting}
@FunctionalInterface
...
Functional interfaces have only one abstract method with \code{@FunctionalInterface} annotation. There is no ambiguity about which method is being overridden by an implementing subclass.  The type can be inferred, simplify the instantiation of the class into a lambda expression!
\begin{lstlisting}
...
\subsection{Delayed computation with lambda functions}
\begin{lstlisting}
@FunctionalInterface
...
\section{9. Monad}
\begin{itemize}
\item A monad is a structure with at lest two methods of and flatMap that obeys three laws. Contains a value + side information
\item \textbf{\code{of}} method to initalize the value \& side information
\item \textbf{\code{flatMap}} method to update value \& side information (takes in some function)
\end{itemize}

\subsection{Monad Laws}
\subsubsection{Left Identity Law:}
\begin{itemize}
\item \code{Monad.of(x).flatMap(y -> f(y))} is equivalent to \code{f(x)}.
\item Basically that flatMap does not modify \code{f(x)}.
\end{itemize}

\subsubsection{Right Identity Law:}
\begin{itemize}
\item \code{monad.flatMap(y -> Monad.of(y)} is equivalent to \code{monad}.
\item Basically that \code{.of()} should behave like an identity.
\end{itemize}

\subsubsection{Associative Law:}
\begin{itemize}
\item \code{monad.flatMap(x -> f(x)).flatMap(x -> g(x))} is equivalent to \code{monad.flatMap( x -> f(x).flatMap(x -> g(x)) )}.
\item Aka same result regardless of how it is composed.
\end{itemize}

\section{Functor}
\begin{itemize}
\item Has two methods \code{of} and \code{map}, does not carry side information.
\end{itemize}
\subsection{Functor Laws}
\subsubsection{Preserving Identity}
\begin{itemize}
\item \code{functor.map(x -> x)} is same as \code{functor}
\end{itemize}
\subsubsection{Preserving Composition}
\begin{itemize}
\item \code{functor.map(x -> f(x)).map(x -> g(x))} is same as \code{functor.map(x -> g(f(x))}
\end{itemize}

\vfill\null
\columnbreak

\section{InfiniteList}
\begin{itemize}
\item Lazy evaluation allows us to delay the computation that produces data until the data is needed. This powerful concept enables us to build computationally-efficient data structures. List with a possibly infinite number of elements.
\item we can delay a computation by using the Producer functional interface (or anything equivalent).
\end{itemize}
...
\end{lstlisting}
\includegraphics[width = \linewidth]{evens}

\begin{lstlisting}
InfiniteList<Integer> odds = evens.map(x -> x + 1);
\end{lstlisting}
\includegraphics[width = \linewidth]{evensMap}

\begin{lstlisting}
InfiniteList<Integer> altEvens = odds.map(x -> x * 2);
\end{lstlisting}
\includegraphics[width = \linewidth]{altEvens}

\vfill\null
\columnbreak

\section{10. Streams}
\subsection{Streams Pipelines}
\begin{itemize}
\item \textbf{Source, Intermediate, Terminal}
\centerline{\includegraphics[width = \linewidth]{streamsPipeline}}
\item \textbf{consumed only once}: Limitation, \code{IllegalStateException} if consumed again.
\item Makes code more declarative compared to how to do it, more succinct, less bug-prone.
\end{itemize}

\section{Parallel Streams}
\begin{itemize}
\item \textbf{Concurrency:} dividing the computation, separate unrelated tasks.
\item \textbf{Parallelism:} While concurrency is illusion, parallel computing is the scenario where multiple subtasks are truly running at the same time → multiple cores/processors. 
\item add \code{.parallel()} before the terminator or use \code{parallelStream()}

\subsubsection{What can be Parallelized?}
\item \textbf{Stateless: Not Stateful:} Result should not depend on any state that may change during exectution of stream (e.g. \code{generate}, \code{generate} operations. Not suitable to run in parallel.
\item \textbf{Not interfering with stream data}: Stream operations that modify source of stream during execution of terminal operation throws \code{ConcurrentModificationException}
\item \textbf{Side Effects} →  use \code{collect()} for ArrayList to avoid.

\subsubsection{Associativity}
\item \textbf{Accumulator}: takes all the tasks in the batch and aggregates it (intra) 
\item \textbf{Combiner}: aggregate across task. (inter)
\item Operation is parallelisable by reducing each substream and combining them with a \code{combiner} if:
\item \code{combiner.apply(identity, i)} is equal to \code{i}.
\item \code{combiner} and \code{accumulator} are associative (order does not matter)
\item \code{combiner} and \code{accumulator} are compatible. aka \code{combiner.apply(u, accumulator.apply(identity, t))} is equal to \code{accumulator.apply(u, t)}

\subsubsection{Ordered vs. Unordered Source}
\item \code{ordered} → created from \code{iterate} / \code{of} / ordered collections like \code{list}
\item \code{unordered} → created from \code{generate} / ordered collections like \code{set}
\item \code{distinct} , \code{sorted} preserve order (stable). Only for finite streams.
\item use \code{.unordered()} to make parallel operations more efficient, as there is no need to coordinate between streams to maintain order. 
\end{itemize}
