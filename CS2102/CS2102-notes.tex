...
• \textbf{Predicate or first order logic}, consists of formulae built from predicates (lowercase) , operators ($=$, etc.) constants (lower case), variables (upper case, quantifiedor free), connectives ($\rightarrow $), and quantifiers ($\forall$ and $\exists$).
• Existential quantifier, $\exists$, is a generalisation of disjunction. \\
• The universal quantifier, $forall$, is a generalisation of conjunction. \\
•  De Morgan laws can be generalised to quantifiers. 
\centerline{\includegraphics[width=0.5 \linewidth]{predicatelogic}}

\subsubsection{Tuple Relational Calculus}
• In \textbf{Tuple Relation Calculus (TRC)} variables values of rows of tables (t-uples.), is the theoretical basis of SQL. \\
• \textbf{Material Implication:} $P \rightarrow Q \equiv \neg P \vee Q$

\subsubsection{Relational Algebra}
• Relations are sets of tuples, main algebraic operators are union, intersect, non-symmetric difference, renaming, selection, projection, cross, and, join etc. \\
• \textbf{Renaming $\rho$} can change name of relation, of attributes or both. \code{xx AS yy} \\
• \textbf{Projection $\pi$} projects attributes given by a list of attributes from relation. \code{(SELECT xx, yy FROM games)} \\
• \textbf{Cross Product $\times$} combines rows and keeps columns. \code{(SELECT* FROM R1, R2;)} \\
• \textbf{Join $\Join$}: R1 $\Join$ R2 = $\sigma$ (R1 $\times$ R2) \\
• Relational algebra is the basis of the implementation of relational DBMS: SQL queries translated into execution plans that orchestrate physical implementations of relational algebra operators.
