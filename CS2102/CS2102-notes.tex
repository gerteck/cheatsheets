...
\subsection{Set of Attributes in closure of X w.r.t F algo[15.1]}
...
\subsection{Computing Minimal Sets of FDs Algo [15.2]}
...
\subsubsection{Normal Forms Summary}
\medskip
\centerline{\includegraphics[width = 1\linewidth]{normalsummary}}
...
\item As long as relation has a key, it is considered to be in 1NF.
\end{itemize}
\medskip
...
\section{15. Relational DB. Design Algorithms}
Focus: Relational Database Design from bottom up, relational synthesis of relations instead of top down decomposition.

\subsection{Design by Analysis (``Top-down'')}
\begin{itemize}
\item \textbf{Normalization} process, decomposing unsatisfactory
relations by breaking up their attributes into smaller relations.
\item ``Improvement” of given design to make it better to eliminate anomalies, redundancy.
\item \textbf{Normal form}: Metric for condition of relation, using keys and FDs of a relation. 
\end{itemize}

\subsection{Design by Synthesis (``Bottom-Up'')}
\textbf{Relational Synthesis Approach}: Synthesizing 3NF relations from functional dependencies (bernstein, 1976).
\begin{itemize}
\item Assumes that all possible functional dependencies 
are known:
\item Imagine all attributes thrown into a single relation: called \textbf{Universal relation}.
\item Construct a \textbf{minimal set of FDs}, then apply algorithms that construct a target \textbf{set of 3NF or BCNF relations}.
\item Additional criteria may be needed to ensure set of relations in relational database are satisfactory (Algorithms 15.3, 15.4). 
\end{itemize}

\medskip
\centerline{\includegraphics[width = 1\linewidth]{DesignProcess}}


\subsection{Designing a set of Relations}
\textbf{Goals}:
\begin{itemize}
\item \textbf{Lossless (non-additive) join property (must)}: Algo 15.3 tests for general losslessness of n-ary decompositions.
\item \textbf{Dependency preservation property}: Observe as much as possible, algo 15.5 decomposes a relation into BCNF components by sacrificing the dependency preservation but guarantees losslessness.
\end{itemize}


\columnbreak

\subsection{Algorithm to determine key of Relation [15.2a]}
\textbf{Algo 15.2a}: Finding a key $K$ for $R$, given a set $F$ of functional dependencies. \\
\textbf{Input}: Universal relation R and a set of functional dependencies F on  attributes of R.
\begin{enumerate}
\item Set $K$ $:=$ $R$
\item For each attribute $A$ in $K$:
	\begin{itemize}
	\item Compute $(K - A)^+$ with respect to F
	\item If closure $(K - A)^+$ contains all attributes in R, then set $K$ $:=$ $K - \{A\}$, aka remove from key. 
	\end{itemize}
\end{enumerate}
\smallskip
\begin{itemize}
\item \textbf{Idea:} Assume default key is entire table, remove an attribute at a time, compute cover and see if still equal original functional dependencies. We end up with the minimal set of attributes, and there may be multiple minimal sets (candidate keys).
\end{itemize}
\medskip
\centerline{\includegraphics[width = 0.65\linewidth]{determineKey}}
...
\subsection{Test Non-Additive Join Property for Binary D.}
\begin{itemize}
\item See NJB test on previous page. Basically: \\
\centerline{(R1 INTERSECT R2) → (R1 - R2) or (R2 - R1)}.
\end{itemize}

\subsection{Algorithm[15.3] Test Non-Additive Join Property for N-ary Decomposition}
\centerline{\includegraphics[width = 1\linewidth]{algo15.3}}

\columnbreak

\subsubsection{N-ary Non-Additive Test Example}
\centerline{\includegraphics[width = 1\linewidth]{nonadditiveexample}}


\subsection{Relational Decomposition}
\textbf{See textbooks for proof.}
\begin{itemize}
\item \textbf{Claim 1 (Dependency Preserving)}: Always possible to find a dependency preserving decomposition $D$ with respect to $F$ such that each relation $R_i$ in $D$ is in 3NF.
\item \textbf{Claim 2 (Preservation of non-additivity in successive decompositions)}: If $D$ = \{R1, R2, ..., Rm\} of $R$ has lossless join property w.r.t. set of FDs $F$ on $R$, and if decomposition $D_i$ = \{Q1, Q2, ..., Qk\} of $R_i$ has lossless join property w.r.t. projection of F on $R_i$, then decomposition $D2$ = \{R1, R2, ..., Ri-1, Q1, Q2, ..., Qk, Ri+1, ..., Rm\} of $R$ has the non-additive join property w.r.t. $F$.
\end{itemize}
...
\subsection{Relational Synthesis Algorithm[15.4] (to 3NF)}
\centerline{\includegraphics[width = 0.95\linewidth]{relationalsynthesis}}
\subsubsection{Algo[15.4] Relational Synthesis Example}
\centerline{\includegraphics[width = 0.95\linewidth]{algo15.4ex}}


\subsubsection{Algo[15.4] Relational Synthesis Example 2}
\centerline{\includegraphics[width = 1\linewidth]{algo15.4ex2}}

\subsection{Relational Synthesis Algorithm[15.5] (to BCNF)}
\textbf{Cannot guarantee functional dependency preservation.} \\
\centerline{\includegraphics[width = 0.95\linewidth]{algo15.5}}

\subsubsection{Algo[15.5] Relational Synthesis Example}
\centerline{\includegraphics[width = 1\linewidth]{algo15.5ex}}

\subsection{Notes on Normalization Algorithms}
\textbf{Problems}:
\begin{itemize}
\item Database designer must first specify all relevant functional dependencies among the database attributes. 
\item Algorithms are not deterministic in general. 
\item Not always possible to find a decomposition into relation schemas that preserves dependencies and allows each relation schema in 
the decomposition to be in BCNF (instead of 3NF as in Algorithm 15.5). 
\end{itemize}

\subsection{Summary: Relational DB Schema Design Algos}
\centerline{\includegraphics[width = 1\linewidth]{algosummary}}
