...
\end{itemize}
\bigskip
\centerline{\includegraphics[width=0.7 \linewidth]{cli}}
\bigskip
...
\subsubsection{Stored Functions: Basic Syntax}
\centerline{\includegraphics[width=0.8 \linewidth]{functionbasicsyntax}}
\bigskip
...
$$ LANGUAGE <sql | plpgsql>;
    ...
    \begin{minipage}{0.40\columnwidth}
      \begin{itemize}[leftmargin=*]
        \item \code{CREATE}  \code{OR REPLACE} helps to re-declare function/procedure if already previously defined
        \item Code is enclosed within \code{\$\$}
        \item Call a function: \\
        \textbf{SELECT * FROM swap(2, 3);}
        \item Call a procedure: \\
        \textbf{CALL transfer( 'Alice', 'Bob', 100);}
      \end{itemize}
    \end{minipage}
...
\subsubsection{Return and Type}
    \begin{center}
      \begin{tabular}{ |c|c| }
        ...
        \makecell{Single tuple \\ from table} & \code{<table_name>} \\ \hline
        \makecell{Set of tuples \\ from table} & \code{SET OF <table_name>} \\ \hline
        Single new tuple & \code{RECORD} \\ \hline
        Set of new tuples & \makecell{ \code{SET OF RECORD} or \\ \code{TABLE(c VARCHAR, x INT)} } \\ \hline
...
\medskip
   ...
   \begin{multicols*}{2}
      \paragraph{Variables}
        \begin{itemize}[leftmargin=*]
          \item \code{DECLARE} [$var, type$] (1 or more when \code{DECLARE} keyword is present)
          \item \code{<var> := <expr>}
        \end{itemize}
    \columnbreak
      \paragraph{Block}
...
\subsubsection*{Examples} \noindent
      Note: \code{INOUT} specifies that the param is both an input and output param
      \begin{multicols*}{2} \noindent
        \textbf{Function}
        \begin{lstlisting}
...
\begin{itemize}
\item Procedures can \textbf{commit or roll back transactions} during execution, functions can not.
\item Unlike functions, \textbf{procedures cannot be involved in DML} commands (select, insert, update, delete)
\item Procedures are invoked in isolation using \code{CALL} (functions always invoked in \code{SELECT} statements.
...
\item \textbf{Assignment of query result} to declared variable(s): \code{SELECT} ... \code{INTO} ...
\end{itemize}

\subsubsection{Errors \& Messages}
\begin{itemize}
\item \code{RAISE}
\item 6 different raise levels in PostgreSQL.
\end{itemize}
\centerline{\includegraphics[width=0.9 \linewidth]{raise}}

\subsubsection{Cursors}
\begin{itemize}
\item  \textbf{Purpose}: Encapsulates query, allows access to each individual row return by a SELECT. (instead of executing whole query to get complete result at once)
\item \textbf{Avoids memory overrun} when the query result is very large. 
\item \textbf{Advantage}: Flexible `navigation' through query results in \textbf{different directions'}. \code{FETCH} to move row and read data, \code{MOVE} only to move to row (no read)
\end{itemize}
...
\centerline{\includegraphics[width=1 \linewidth]{cursorex2}}
    ...
    \begin{center}
      \includegraphics[width=1\columnwidth]{cursor_question}
      \includegraphics[width=1\columnwidth]{cursor_example}
    \end{center}
