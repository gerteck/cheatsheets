...
\subsubsection{Kernel Trick {\scriptsize (Usage of Kernel Method to avoid explicit computation)}}
\vspace{-0.5em}
...
\begin{itemize}[itemsep=-1pt]
	...
	\item \textbf{Maximizes Margins}: SVM model optimized by maximizing margin, subject to constraints that data points are on the correct side of the decision boundary.
...
\subsection{Perceptron {\scriptsize (Binary Classifier)}}
\textbf{Binary classifier}: learns to separate data into two classes based on input features. Fundamental NN building block, inspired by biological brain neurons. 
\begin{itemize}
\item \textbf{Biological neuron}: receives inputs (dendrites), produce output (axon).
\item \textbf{Perceptron}: takes input features, applies weights, takes summation, passes result through an activation function to produce an output.
\end{itemize}
...
\item Equivalently, $h_w(x) = g(w_0x_0 + w_1x_1 + ... + w_dx_d)$
...
\subsubsection{Perceptron Learning Algorithm {\scriptsize (Iteratively update weights to train model)}}
...
\end{enumerate}
\textbf{Example: Cancer Prediction}: Binary classification of tumor.

\centerline{\includegraphics[width=0.95\linewidth]{perceptronLearning}}

\subsubsection{Perceptron Limitations}
\begin{itemize}
\item Solves linearly separable problems, cannot handle non-linear feature relationships.
...
\subsection{\underline{Neural Network}}
\textbf{Multiple layers of interconnected neurons}, model complex/non-linear patterns.
\begin{itemize}
\item \textbf{Applications}: Neural networks widely used for image classification, NLP (sentiment analysis), multi-class classification (predict out of several categories).
\end{itemize}

\subsection{Neuron {\scriptsize (Generalization of a Perceptron)}}
...
\subsection{Feature Engineering {\scriptsize (To model Non-Linearly Separable Problems)}}
\begin{itemize}
\item \textbf{Manual Feature Engineering}: Manual transform existing features to capture non-linear relationships, e.g. polynomial features: $x_1^2, x_2^2, x_1x_2$, exponential or logarithmic transformations: $e^{x_1}, \log(x_2)$, combine features multiplicatively: $x_1x_2$. 
\item \textbf{Using Neurons for Feature Transformation}: Use neuron layers to auto learn new features. Hidden layers apply non-linear activation functions (e.g., sigmoid, ReLU). Weights of neurons effectively create new features making it linearly separable.
\item \textbf{Feature Engineering Usage}: manual feature engineering used in Linear, Logisitic Regression to capture complex patterns. Multi-layer neural network learns on own. 
\item E.g. To model XOR or XNOR gate (not linearly separable), require intermediate hidden layer for preliminary transformation to achieve logic of XOR/XNOR gate. 
\end{itemize}

\subsection{Multi-layer Neural Networks}

\textbf{Three components}: \textit{input layer} (raw features), \textit{hidden layers} (transform features w learned weights, activation functions), and \textit{output layer} (final predictions).
...
\subsubsection{Multi-class Classification: Softmax Function As Activation Function}
For multi-class classification, the \underline{softmax function is used in the output layer} to convert raw scores into probabilities:
\[
\text{\textbf{SoftMax: } \quad}  g(z_i) = \frac{e^{z_i}}{\sum_{j=1}^c e^{z_j}},
\]
where $c$ is the number of classes, and $g(z_1) + g(z_2) + \dots + g(z_c) = 1$.
\begin{itemize}
\item \textbf{Softmax function}: used to convert raw scores $z_i$ (summation before activation) into probabilities, and ensures that output values sum to 1. (Normalizes output to probability distribution over predicted output classes.)
\item \textbf{Usage}: Set number of neurons in last layer as $c$, predict probability of each class using softmax and pick highest probabilty.
\end{itemize}

\subsection{\underline{Neural Networks Training} {\scriptsize (Adjusting weights to minimize loss)}}

\subsection{Gradient Descent}
\centerline{\includegraphics[width=1\linewidth]{multiLayer2}}
\centerline{\includegraphics[width=1\linewidth]{multiLayer3}}

\subsubsection{Forward Propagation {\scriptsize (Input data passed through to compute output)}}
\begin{itemize}[itemsep=0pt]
\item $\hat{y} = g(W^{[L]} \cdot g(W^{[L-1]} \cdot \dots g(W^{[1]} \cdot x) \dots ))$
\item $ W^{[l]} $: Weight matrix for layer $ l $, $ x $: Input features. $ g(\cdot) $: Activation function.
...
\subsubsection{Backpropagation {\scriptsize (Gradient Computation backwards)}}
...
\subsubsection{Gradient Descent: Weight Update Formula {\scriptsize (Using computed gradients)}}
$$
w_j \leftarrow w_j - \gamma \frac{\partial L}{\partial w_j}, \quad \gamma : \text{ learning rate.}
$$
\begin{itemize}
\item \textbf{E.g.}: $\frac{\partial L}{\partial w_3} =  \frac{\partial L}{\partial z_1} \cdot \frac{\partial z_1}{\partial w_3} = [ (\hat{y} - y) \cdot g'(z_3) \cdot w_5 \cdot g'(z_1) ] \cdot [ x_2 ]$
\item \textbf{Iterative Optimization}: Forward propagation, loss computation, backpropagation, and weight update steps repeated iteratively until convergence / stopping criterion.
\end{itemize}

\subsection{\underline{Single-Layer Neural Network} (One Neuron)}
...
\end{enumerate}
\begin{itemize}[itemsep=0pt, topsep=0pt]
\item For \textbf{Binary classification}, activation function is typically sigmoid function. Then, the loss function for a single neuron is often the binary cross-entropy (BCE) loss: $L = -y \log(\hat{y}) - (1-y) \log(1-\hat{y}).$
\item \textbf{Binary Classification Hyperparameters} $\alpha$, $\beta$ usable as weights ($L = \alpha[-y \log(\hat{y})] + \beta[-(1-y) \log(1-\hat{y}]).$ Can mitigate problem of highly unbalanced dataset, so machine wont bias towards predicting all samples as overrepresented class. (Punish misclassification of underrepresented class more)
\item \textbf{Optimization}: For single layer NN, could do manual feature engineering. However, could be computationally expensive, with overfitting if too many features. Final decision still ultimately a linear combination of transformed features.
\end{itemize}


\subsection{\underline{Multi-layer Neural Network}}
Multiple layers of neurons, enables modeling complex, non-linear relationships. 
\begin{itemize}
\item \textbf{Hidden Layers}: Hidden layers transform the input features into higher-level representations:
$$
a^{[l]} = g(W^{[l]} a^{[l-1]} + b^{[l]}),
$$
where $ a^{[l]} $ is the activation of layer $ l $.
\item \textbf{Activation Functions}: Includes ReLU (Rectified Linear Unit): $ g(z) = \max(0, z) $, Sigmoid: $ g(z) = \frac{1}{1 + e^{-z}} $, Softmax (see multi-class classification).
\item \textbf{Output Layer}: produces final prediction. For regression tasks, it uses a linear activation function. For classification tasks, it uses softmax or sigmoid.
\item \textbf{Optimizations of Deep NN}: For a multi-layer NN:
	\begin{itemize}
	\item \textbf{Increase Depth of Network}: Add more hidden layers.
	\item \textbf{Increase Width of Network}: Add more neurons per layer.
	\item Complexity of computed function grows exponential with depth, increase more slowly with width. Increasing depth generally more efficient.
	\item Increased capacity of learning complex patterns also increases risk of overfitting.
	\item \textbf{Others}: All weights not equal (initial layer matters more, more sensitive), lower layer weights less robust to noise but perform better when optimized. (See paper)
	\end{itemize}
\item \textbf{Potential Issues of Deep NN}: 
	\begin{itemize}
	\item \textbf{Sigmoid Function}: to compute derivative of first few layers (e.g. $\frac{\partial \varepsilon}{\partial W^{[1]}}$ ), it is product of many derivatives, which is each between 0 and 1. Known as \textbf{Vanishing Gradient problem}. 
	\item To mitigate, we can use ReLU function, as long as input is non-negative, derivative is 1, precluding the issue.
	\item \textbf{ReLU function}: When majority of activations are 0 (underlying pre-activation summation mostly non-positive) - network dies halfway. Gradients passed back are 0, cannot recover, stops learning. Known as \textbf{Dying ReLU problem}.
	\item To mitigate, use Leaky ReLU, has small positive gradient for negative inputs, gives network chance to recover.
	\end{itemize}
\end{itemize}
...
\item \textbf{Convolution}: math operation, combines 2 functions to produce third function. 
	\begin{itemize}
...
\begin{itemize}
\item {\tiny \textbf{PyTorch}:  \texttt{conv\_layer = nn.Conv2d(in\_channels, out\_channels, kernel\_size, stride, padding)}}
\end{itemize}
...
\item For a single image and a single kernel, output height $H_1 = \lfloor \frac{H-K+2P}{S} \rfloor + 1$.
\end{itemize}

\subsubsection{Pooling Layer}
Pooling layers \textbf{downsample} feature maps to reduce dimensionality.
\begin{itemize}
...
\subsection{13. \underline{Recurrent Neural Networks (RNNs)} {\scriptsize (Processes Sequential Data)}}
\begin{itemize}
\item \textbf{Sequential data}, e.g. text, audio, has \textbf{patterns, dependencies} across time. Elements treated sequentially to capture relationships.
\item \textbf{Hidden State}: RNNs maintain a \textbf{hidden state}, unlike traditional feedforward networks. It captures info of previous elements in sequence, to encode contextual info and model temporal dependencies.
...
\item \textbf{Example}: `we saw this saw.', use one-hot encoding, represent word as vectors. Predict`saw' is verb or noun requires context of previous words.
\end{itemize}
...
\end{itemize}
\centerline{\includegraphics[width=0.95\linewidth]{RNN}}
\hrule
...
\end{itemize}
\centerline{\includegraphics[width=1\linewidth]{many-to-many-1}}
\begin{itemize}
...
\end{itemize}
\centerline{\includegraphics[width=1\linewidth]{many-to-many-2}}
...
\centerline{\includegraphics[width=1\linewidth]{deepRNN}}
...
\begin{itemize}
	...
	\item \textbf{Properties of RNN}: \textit{Capture Contextual Information}: RNNs use hidden state to encode information about entire sequence seen so far. \textbf{Sequential in Nature}: Predictions at time step $t$ depend on all previous steps, less parallelism-friendly.
\end{itemize}
...
\centerline{\includegraphics[width=1\linewidth]{matrixRNN}}
...
\end{enumerate}
%% Example
\centerline{\includegraphics[width=1\linewidth]{selfAttentionLayer}}
...
\item \textbf{Applications}: Self-attention core component of transformer architectures, widely used in NLP, (machine translation, text summarization), Computer Vision (image classification etc), Speech Processing (speech recognition, synthesis).
\item \textbf{Still requires Positional Encoding}: One-hot encoding to represent words, input vectors to self-attention layer, generate outputs. But, \textbf{positional encoding} still needed to generate diff outputs for different unique positional instances.
\end{itemize}
...
\item \textbf{Positional Encoding Purpose}: Enable self-attention layers to distinguish btw. identical tokens at different positions. (e.g. used with one-hot encoding)
\item Positional encoding ensures sequential models capture order of elements in input sequence. Particularly important in architectures like Transformers, which do not inherently process sequences in order.
\end{itemize}

\subsection{\underline{Deep Learning Issues / Problems}}
...
\item \textbf{Solutions to Overfitting}:
	\begin{itemize}
	\item \textbf{Dropout}: During training, randomly set some neurons' outputs to zero with a given probability. Prevents model from relying too heavily on specific neurons.
	...
	\item \textbf{Regularization}: Add penalty terms (e.g., L1, L2 regularization) to loss function.
...
\subsubsection{Vanishing and Exploding Gradients {\scriptsize (Specific to Backpropagation)}}
\begin{enumerate}
\item \textbf{Vanishing Gradient}: Small gradients multiplied repeatedly in backpropagation through layers, tend to 0. Prevents earlier layers from learning effectively.
	\begin{itemize}
	\item \textbf{Mitigation Strategies for Vanishing Gradients}: Use activation functions that avoid saturation (e.g., ReLU instead of sigmoid or tanh), Normalize inputs and use techniques like Batch Normalization to stabilize gradients.
	\end{itemize}
\item \textbf{Exploding Gradient}: Large gradients get multiplied repeatedly, causing numerical instability or overflow. Destabilizes training process.
	\begin{itemize}
	\item \textbf{Mitigation Strategies for Exploding Gradients}: Apply gradient clipping: Clip gradients to a specified range (e.g., $[-\text{clip\_value}, \text{clip\_value}]$). Use weight initialization techniques to prevent large initial gradients.
	\end{itemize}
\end{enumerate}

\subsection{\underline{Attention Neural Networks:} Types}
\begin{itemize}
\item \textbf{Many-to-Many Attention}: where input and output sequences same length.
\item \textbf{Many-to-One Attention}: Input sequence processed to produce a single output.
	\begin{itemize}
	\item \textbf{Added Input}: \texttt{<SUM>}, which is learnable input token vector that triggers summarization. It is added to capture summary representation.
	\item \textbf{Added hidden state}: $h_{\text{SUM}}$ is the hidden state that captures summarized sequence, $h_{\text{SUM}} = \sum_j \alpha'_j v_j$, where  $\alpha'_j $ are normalized attention weights.
	\item Why need new hidden state? Each hidden state $ h_t $ only aggregates contextually relevant info, but not summarize entire sequence. $h^t$ is position-specific focused, summary $h^{\texttt{SUM}}$ captures global context.
	  \item (tasks e.g. sentiment analysis, classification).
	\end{itemize}
\end{itemize}
\centerline{\includegraphics[width=1\linewidth]{attentionNN1}}
...
\item \textbf{One-to-Many Attention}: A single input ($ x_1 $) generates sequence of multiple outputs, where $ T_x = 1 $ and $ T_y > 1 $ (e.g. text generation).
	\begin{itemize}
	\item \textbf{Algorithm}: Generate 1st output $ \hat{y}_1 $ via hidden state $ h_1 $. Subsequent $ \hat{y}_2, \hat{y}_3, \dots $ generated sequentially, incorporate previous predictions $ \hat{y}_1, \hat{y}_2, \dots $ as input.
...
\item \textbf{\texttt{ADD} mask} after computing attention scores, prevent attending to future tokens (cheating). Maintains causal prediction, as during training, in techniques like teacher forcing, ground truth tokens ($ y_1, y_2, \dots $) provided. 
\item Without mask, model could "cheat" by attending to future tokens, leading to overfitting and poor generalization during inference.
	\end{itemize}
\end{itemize}
\centerline{\includegraphics[width=1\linewidth]{attentionNN2}}
 
 % --------- 
  
\subsubsection{Many-to-Many Attention {\scriptsize (input/output sequence different lengths)}}
\centerline{\includegraphics[width=1\linewidth]{attentionNN3}}
\begin{itemize}
...
\item \textbf{Cross-Attention Layer}: enables decoder to focus on relevant parts of encoder's output while generating tokens in out sequence. Computes attention scores using:
	\begin{itemize}
	...
	\end{itemize}
\end{itemize}
\centerline{\includegraphics[width=1\linewidth]{crossAttention}}

\subsection{\underline{Transformers} {\scriptsize (Deep attention-based Neural Networks).}}
\begin{itemize}
...
\item \textbf{Basic Building Blocks}:
	...
	\item \textbf{Masked Self-Attention Layer}: Ensures causality in autoregressive tasks.
	\item \textbf{Cross-Attention Layer}: Enables decoder to focus on parts of encoder's output.
	\item \textbf{Linear Layers}: Used for projection and transformation of inputs.
	\end{itemize}
\end{itemize}
\centerline{\includegraphics[width=1\linewidth]{transformer}}
...
\item \textbf{Applications of Transformers}:
	...
	\item \textbf{Vision Transformers (ViT)}: Adapt Transformers for image classification by treating image patches as sequence elements. ($+$ Positional Encoding!) 
	\item \textbf{Others}: machine translation, speech recognition, time-series forecasting etc.
	\end{itemize}
\end{itemize}
...
\item \textbf{Non-linear Activation Functions}:
	...
	\item \texttt{torch.nn.Softmax}: Softmax activation function.
	\end{itemize}
\item \textbf{Loss Functions}: Various for diff. tasks:
	...
	\item \texttt{torch.nn.CrossEntropyLoss}: Cross-Entropy Loss, multi-class classifn.
	\end{itemize}
...
\centerline{\includegraphics[width=\linewidth]{pytorch}}
\subsubsection{Training a Neural Network Example}
\centerline{\includegraphics[width=\linewidth]{pytorch2}}
