...
Each segment is now composed of pages and has a page
table of its own. The segment table now points to the
page table address instead of the base address. Page
limit remains unchanged.


\section{7.5 Virtual Memory Management}
 Use of secondary storage as an extended memory region. Remove assumption, physical memory (RAM) may not be large enough to hold one or more processes’
logical memory space completely.
\begin{itemize}
\item A popular solution is to keep some pages in the physical memory, while others on secondary storage or disks (HDD, SDD). Bringing the pages
in and out of memory is called \textbf{swapping}.
\item \textbf{Logical not restricted by physical}: Logical memory address size is no longer restricted by physical
\item \textbf{More processes in memory}: Improves CPU utilisation since more processes to choose to run
\end{itemize}
...
\item (Hardware) Page fault: TRAP to OS
\item (OS) Locate page in secondary storage
\item (OS) Load copy of page in physical memory frame
\item (OS) Update page table
\item (OS) Go back to step 1 to retry
\end{enumerate}
